...
//std::cout<<"TCP "<<std::endl;
  ...
  // is pending?
//shared_ptr<Data> mydata = makeData(interest.getName());
//shared_ptr<const Data>;
//mydata.setContent();
//data(interest.getName());
//m_cs.insert(data);
//*////**********************
/////oncontent store hit if it finds smth
  if (!pitEntry->hasInRecords()) {
//std::cout<<"DO U KNOW THE WAY :  ";
    if (m_csFromNdnSim == nullptr) {
      m_cs.find(interest,
                bind(&Forwarder::onContentStoreHit, this, ref(inFace), pitEntry, _1, _2),
                bind(&Forwarder::onContentStoreMiss, this, ref(inFace), pitEntry, _1));
testing_new_feature();
//std::cout<<"SHOW ME THE WAY  : "<< ref(inFace)<<std::endl;
//std::cout<<"test : "<< interest<<std::endl;
    }
    else {
std::cout<<"WTF IS FINISHED   "<<interest.getName()<<std::endl;
//interest="a";
      m_cs.find(interest,
                bind(&Forwarder::onContentStoreHit, this, ref(inFace), pitEntry, _1, _2),
                bind(&Forwarder::onContentStoreMiss, this, ref(inFace), pitEntry, _1));
...
std::cout<<"WTF IS FINISHED ?"<<std::endl;
    this->onContentStoreMiss(inFace, pitEntry, interest);
  }
}
...
Forwarder::onIncomingData(Face& inFace, const Data& data)
{
  ...
  // CS insert
  if (m_csFromNdnSim == nullptr)
    m_cs.insert(*dataCopyWithoutTag);//////////////////start
  else
